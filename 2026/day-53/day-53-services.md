...
test-client ( pod ) → service ( clusterIP ( service yaml file ) ) → one of pods ( of web app ( with 3 replica))

Try multiple times

You hit different pods (load balancing)

service IP is static and the traffic will go from service so even if the pods IP changes no problem 
...
<img width="1704" height="716" alt="image" src="https://github.com/user-attachments/assets/83bfb001-e8c1-4a65-9cbe-ba643efbb3c7" />

<img width="1880" height="1108" alt="image" src="https://github.com/user-attachments/assets/b0a5eaea-c2eb-4b5e-9022-08557e91c42b" />

The IP address of service and the response of nslookup are same.
...
-  ClusterIP only works inside cluster

Solution → NodePort

accessed from browser:

<img width="2282" height="736" alt="image" src="https://github.com/user-attachments/assets/bc6e1191-2362-477a-ad7c-fb1a49fb5a83" />

Port mapping since the cluster is runnning in kind and the whole cluster is running inside a docker container and outer world does not have 
access to it:

<img width="2228" height="258" alt="image" src="https://github.com/user-attachments/assets/6ca91e66-59eb-4774-9c7e-7306b200221f" />
...
Why pending?
 You are not on AWS / GCP
 No real load balancer

 <img width="1924" height="272" alt="image" src="https://github.com/user-attachments/assets/28d3fcd9-69c8-4640-95ab-f5d664b8bc31" />
...
LoadBalancer | real external

LoadBalancer = NodePort + ClusterIP is assigned 

Additional:

Endpoints:

<img width="1990" height="184" alt="image" src="https://github.com/user-attachments/assets/644dafee-6297-414c-863b-d2a50411d775" />
...
With Service
  ...
  One stable name
  Load balancing
  No worry about pod changes

  Service = stable entry point + load balancer for pods
